...
trait LabUtilsForScalaFx {
  ...
  /***
   * 如果检测到鼠标按键 testButton，那么执行操作 op
   *
   * @param testButton MouseButton 类，NONE 表示鼠标移动，需要实验 Form 类支持此事件传递
   *                   MouseButton.PRIMARY 表示鼠标左键
   *                   MouseButton.SECONDARY 表示鼠标右键
  ...
  /***
   * 闭包版本：如果检测到鼠标按键 testButton，那么执行操作 op
   *
   * @param testButton MouseButton 类，NONE 表示鼠标移动，需要实验 Form 类支持此事件传递
   *                   MouseButton.PRIMARY 表示鼠标左键
   *                   MouseButton.SECONDARY 表示鼠标右键
  ...
  /**
   * 如果检测到键盘按键 testButton，那么执行操作 op
   * @param testButton KeyCode 类，比如 KeyCode.SPACE 空格键
   * @param event 侦听到的事件
   * @param exp 当前实验
   * @param scene 当前场景
  ...
  /**
   * 闭包版本：如果检测到键盘按键 testButton，那么执行操作 op
   * @param testButton KeyCode 类，比如 KeyCode.SPACE 空格键
   * @param event 侦听到的事件
   * @param op 需要执行的操作
   */
  ...
  def ifKeyIn(event: Event)(op: PartialFunction[KeyCode, Unit]):Unit = event match {
    case keyEvent: KeyEvent => op(keyEvent.getCode)
    case _ =>
  }
  ...
  /**
   * 在此方法中进行 GUI 的更改是安全的，因为代码在执行之前，会首先判断当前屏幕是否为指针指向的屏幕，如果不是，则不执行
   * 此方法可用在 Timer 设置的定时器中，定时改变 GUI 界面
   * 请最好不要在 Psy4J 的 Screen 中使用 Timer 进行定时改变 Screen 的 GUI 界面
   * @param op 操作
   * @param experiment 当前实验对象
